...
router.get('/:id', (req, res) => {
  connection.query(
    ...
    (err, results) => {
      if (err) {
        console.log(err)
        res.status(500).send('Error retrieving data from health_insurance')
      } else {
        res.status(200).json(results)
      }
    }
...
router.post('/', [check('insurance_name').isLength({ min: 3 })], (req, res) => {
...
router.put(
  '/:id',
  [check('insurance_name').isLength({ min: 3 })],
  (req, res) => {
    ...
    return connection.query(
      'UPDATE Health_insurance SET ? WHERE id_insurance=?',
...
router.delete('/:id', (req, res) => {
  connection.query(
    ...
    err => {
      if (err) {
        console.log(err)
        res.status(500).send('Error deleting health insurance data')
      } else {
        res.status(200).send('health insurance sucessfuly deleted !')
      }
    }
  )
})
